...
//      this.est = null;
  ...
  getResp() async{
    ...
    var resps = await db.query("SELECT * FROM RespuestasVisita WHERE visitasId = ${this.vId}");
    this.resps = resps;
...
//      print(respuestas[i]);
    ...
    Map<String,dynamic> bloques = (est['bloques'] is List)?Map():est['bloques'];
...
//        print('BBBBBAREAS: ${b['areas'].runtimeType}');
        ...
        Map<String,dynamic> areas = b['areas'].runtimeType == AAA.runtimeType ? Map<String,dynamic>():b['areas'];
            ...
            switch(p['tipo']){
              case 'sub':
                ...
                subpregs.forEach( (spId,sp) {
                  var ii = respuestas.indexWhere((r) => r['identificador'] == spId);

                  est['bloques'][bId]['areas'][aId]['preguntas'][pId]['subpregs'][spId]['respuesta']
                  = (ii == -1)? null:respuestas[ii]['respuesta'];
                });

                break;
              case 'mult':
                var ii = respuestas.indexWhere((r) => r['identificador'] == pId);
//              print('pId = $pId');
//              print('ii = $ii');
                est['bloques'][bId]['areas'][aId]['preguntas'][pId]['respuesta'] = (ii == -1)?null:respuestas[ii]['respuesta'];
        ...
        pregs.forEach((pidentif,p){
          ...
          switch (p['tipo']) {
            case 'mult':
              if(p['respuesta'] != null){
                ...
                preguntas[pidentif]['respuesta'] = (ii == -1)?'_':respuestas[ii]['respuesta'];//$resp['valor'];
                preguntas[pidentif]['valResp'] = (ii == -1)?'_':respuestas[ii]['valResp'];//$resp['valor'];
                preguntas[pidentif]['nomResp'] = (ii == -1)?'_':respuestas[ii]['rNom'];//$resp['respuesta'];
                preguntas[pidentif]['justificacion'] = (ii == -1)?null:respuestas[ii]['justificacion'];//$resp['respuesta'];
            ...
            case 'num':
              ...
              preguntas[pidentif]['respuesta'] = (ii == -1)?'_':respuestas[ii]['respuesta'];
              preguntas[pidentif]['valResp'] = (ii == -1)?'_':respuestas[ii]['valResp'];
              preguntas[pidentif]['nomResp'] = (ii == -1)?'_':respuestas[ii]['valResp'];
              preguntas[pidentif]['justificacion'] = (ii == -1)?null:respuestas[ii]['justificacion'];//$resp['respuesta'];
          ...
          if(p['tipo'] == 'sub'){
            ...
            subpregs.forEach((spidentif,sp){
              ...
              preguntas[spidentif]['subareaNom'] = preguntas[pidentif]['pregunta'];
              preguntas[spidentif]['muestra'] = 1;

              var ii = respuestas.indexWhere((r) => r['identificador'] == spidentif);
              switch (sp['tipo']) {
                case 'mult':
...
//                    }
                    // $resp = $db->query("SELECT * FROM Respuestas WHERE id = $p[respuesta]")->fetch(PDO::FETCH_ASSOC);
                    preguntas[spidentif]['respuesta'] = (ii == -1)?'_':respuestas[ii]['respuesta'];//$resp['valor'];
                    preguntas[spidentif]['valResp'] = (ii == -1)?'_':respuestas[ii]['valResp'];//$resp['valor'];
                    preguntas[spidentif]['nomResp'] = (ii == -1)?'_':respuestas[ii]['rNom'];//$resp['respuesta'];
                    preguntas[spidentif]['justificacion'] = (ii == -1)?null:respuestas[ii]['justificacion'];//$resp['respuesta'];
                ...
                case 'num':
                  ...
                  preguntas[spidentif]['respuesta'] = (ii == -1)?'_':respuestas[ii]['respuesta'];
                  preguntas[spidentif]['valResp'] = (ii == -1)?'_':respuestas[ii]['valResp'];
                  preguntas[spidentif]['nomResp'] = (ii == -1)?'_':respuestas[ii]['valResp'];
                  preguntas[spidentif]['justificacion'] = (ii == -1)?null:respuestas[ii]['justificacion'];//$resp['respuesta'];
...
//      }
      ...
      preguntas[identificador]['valPreg'] = is_numeric('$valpreg')?'$valpreg':'_';
...
//    print(' Identificador $identificador, Pregs: $pregs');
    ...
    if(pregs[identificador] != null &&
        (pregs[identificador]['valPreg'] != null && pregs[identificador]['valPreg'] != '_' && pregs[identificador]['valPreg'] != '-' )
    && false){

      valor = is_numeric('${pregs[identificador]['valResp']}' ) &&  is_numeric('${pregs[identificador]['puntos']}')?
        num.parse('${pregs[identificador]['valResp']}')*num.parse('${pregs[identificador]['puntos']}'):
        '_';
...
//            print('PPPPP: ${p['subpregs']}');
    ...
    pregs.forEach((pId,p){
      ...
      puntos = (is_numeric(p['valPreg']) && p['influyeValor'] == '1')?num.parse('${p['puntos']}'):0;
      valPreg = (is_numeric(p['valPreg']) && p['influyeValor'] == '1')?num.parse('${p['valPreg']}'):'_';
      ...
      if( is_numeric('${r['bloques'][p['bloque']]['conseguido']}') ){
        ...
        if( is_numeric('$valPreg') ){
          r['bloques'][p['bloque']]['conseguido'] = num.parse('${r['bloques'][p['bloque']]['conseguido']}');
    ...
    r['bloques'].forEach((bId,b){
          ...
          switch (c['accion']) {
            ...
            case '4':
              if( is_numeric('${r['bloques'][bId]['prom']}') && is_numeric('${c['valor']}') ){
                r['bloques'][bId]['prom'] = num.parse('${r['bloques'][bId]['prom']}')+num.parse('${c['valor']}');
              }
              break;
            default:
              break;
      ...
      b['areas'].forEach((aId,a){
        ...
        if( is_numeric('${a['conseguido']}') ){
          r['bloques'][bId]['areas'][aId]['prom'] = a['max'] > 0 ? num.parse('${a['conseguido']}')/a['max']:0;
        }else{
            ...
            switch (c['accion']) {
              ...
              case '4':
                if( is_numeric('${r['bloques'][bId]['areas'][aId]['prom']}')  && is_numeric('${c['valor']}') ){
                  r['bloques'][bId]['areas'][aId]['prom'] = num.parse('${r['bloques'][bId]['prom']}')+num.parse('${c['valor']}');
                }
                break;
              default:
                break;
        ...
        r['bloques'][bId]['areas'][aId]['prom'] = is_numeric('$prom') ? (num.parse('${a['valMax']}')/100)*num.parse('$prom') : '-';
        ...
        switch (c['accion']) {
          ...
          case '4':
            if( is_numeric('${r['chk']['prom']}') && is_numeric('${c['valor']}') ){
              r['chk']['prom'] = num.parse('${r['chk']['prom']}')+num.parse('${c['valor']}');
            }
            break;
          default:
            break;
    ...
    for(String bId in r['bloques'].keys){
      ...
      r['bloques'][bId]['pond'] = r['bloques'][bId]['max'] > 0 ?
        num.parse('${r['bloques'][bId]['valMax']}')/r['bloques'][bId]['max'] :
        '-';
      ...
      for(String aId in b['areas'].keys){
        ...
        r['bloques'][bId]['areas'][aId]['pond'] =
          r['bloques'][bId]['areas'][aId]['max'] > 0 ?
          num.parse('${r['bloques'][bId]['areas'][aId]['valMax']}')/num.parse('${r['bloques'][bId]['areas'][aId]['max']}') :
          '-';
        switch (b['tipoProm']) {
          case '1':
      ...
      switch (b['tipoProm']) {
        ...
        case '2':
          if(is_numeric('${b['valMax']}') && is_numeric('${r['bloques'][bId]['sumInf']}') ){
            r['bloques'][bId]['prom'] = (num.parse('${b['valMax']}')/100)*num.parse('${r['bloques'][bId]['sumInf']}');
          }else{
            r['bloques'][bId]['prom'] = '-';
          }
          break;
        case '3':
          if(is_numeric('${b['valMax']}') && is_numeric('${r['bloques'][bId]['promInf']}') ){
            r['bloques'][bId]['prom'] = (num.parse('${b['valMax']}')/100)*num.parse('${r['bloques'][bId]['promInf']}');
          }else{
            r['bloques'][bId]['prom'] = '-';
          }
          break;
      ...
      if(is_numeric('${r['chk']['sumInf']}')){
        r['chk']['sumInf'] += is_numeric('${b['prom']}') ? num.parse('${b['prom']}') : 0 ;
      }else{
        r['chk']['sumInf'] = b['prom'];
      }
  ...
  getInstalacion(bool cantidades,String tipo) async {
    ...
    var equipos = await db.query("SELECT * FROM InstalacionesEquipos WHERE instalacionesId = $inst");
...
//    print(instalacion);
  ...
  getInstVis(int instId) async {
    ...
    var equipos = await db.query("SELECT * FROM InstalacionesEquipos WHERE instalacionesId = $instId");
//    print('Equipos $equipos');
    for(int i = 0;i<equipos.length;i++){
//      print('dimensionesElemId = ${equipos[i]['dimensionesElemId']}');
      Map datEle = await datosEquipo(equipos[i]['dimensionesElemId']);
...
//    print('PPPPPP $p');
  ...
  sigSubPreg(bId,aId,pId,spId) async {
    ...
    Map subPreguntas = est['bloques'][bId]['areas'][aId]['preguntas'][pId]['subpregs'];
...
//    print('bId: $bId, aId: $aId');
    ...
    List<dynamic> AAA = List<dynamic>();
    Map<String,dynamic> areas = est['bloques'][bId]['areas'].runtimeType == AAA.runtimeType ? Map<String,dynamic>():est['bloques'][bId]['areas'];
...
//    print('SIGUIENTE $sig $preg');
  ...
  guardaResp(resp) async {
    ...
    print(resp);
    var rvId;
    var rv = await db.query("SELECT * FROM RespuestasVisita WHERE visitasId = ${resp['visitasId']} AND preguntasId = ${resp['preguntasId']}");
    if(rv == null){
      print('-------- ACA -------');
      rvId = await db.insert('RespuestasVisita', resp,true);
    }else{
      rvId = rv[0]['id'];
      await db.query("UPDATE RespuestasVisita SET respuesta = '${resp['respuesta']}', justificacion = '${resp['justificacion']}' WHERE id = $rvId");

    }

    await db.query("UPDATE Visitas SET offline = 1 WHERE id = ${resp['visitasId']}");
...
//          print('Falta preg $i');
  ...
  finalizaVisita({bool viable = true,String justifViable}) async {
    ...
    var datVisDB = await db.query("SELECT * FROM Visitas WHERE id = ${this.vId}");
...
//    print(' - - - - HOY: $hoy - - - - - -');
    String sql = '';
    sql = 'UPDATE Visitas SET finishDate = "$hoy", finalizada = 1, offline = 1 WHERE id = ${this.vId}';
